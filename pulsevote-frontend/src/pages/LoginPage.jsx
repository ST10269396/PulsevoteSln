...
const LoginPage = () => {
  ...
  const isStrongPassword = (password) => /^(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?]{8,}$/.test(password)
  ...
  const validateForm = () => {
    ...
    else if (!isStrongPassword(formData.password)) errors.password = 'Password must be 8+ chars and include letters and numbers'
  ...
  const handleSubmit = async (e) => {
    ...
    if (result.success) {
      if (formData.rememberMe) localStorage.setItem('pulsevote_remember', 'true')
      navigate(from, { replace: true })
    }
    setIsSubmitting(false)
  }

  const isFormValid = formData.email.trim() && formData.password && Object.values(validationErrors).every(v => !v)
      ...
      <div className="card w-full max-w-md bg-base-100 shadow-xl">
        <div className="card-body">
          <div className="text-center mb-6">
            <h2 className="card-title text-2xl font-bold justify-center">Welcome Back</h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-4" noValidate>
            <div className="form-control">
              ...
              {validationErrors.email && (
                <label className="label"><span id="email-error" className="label-text-alt text-error">{validationErrors.email}</span></label>
              )}
            </div>
            ...
            <div className="form-control">
              ...
              <div className="relative">
                <input type={showPassword ? 'text' : 'password'} id="password" name="password" value={formData.password} onChange={handleChange} placeholder="Enter your password" className={`input input-bordered w-full pr-10 ${validationErrors.password ? 'input-error' : 'focus:input-primary'}`} autoComplete="current-password" aria-describedby={validationErrors.password ? 'password-error' : undefined} required />
              ...
              {validationErrors.password && (
                <label className="label"><span id="password-error" className="label-text-alt text-error">{validationErrors.password}</span></label>
              )}
            </div>

            <div className="form-control">
              <label className="label cursor-pointer justify-start gap-3">
                <input type="checkbox" name="rememberMe" checked={formData.rememberMe} onChange={handleChange} className="checkbox checkbox-primary checkbox-sm" />
            ...
            <div className="form-control mt-6">
              <button type="submit" className={`btn btn-primary w-full ${(loading || isSubmitting) ? 'loading' : ''} ${!isFormValid ? 'btn-disabled' : ''}`} disabled={loading || isSubmitting || !isFormValid}>
                {(loading || isSubmitting) ? (<><span className="loading loading-spinner loading-sm"></span>Signing in...</>) : ('Sign In')}
              </button>
            </div>
          </form>
          ...
          <div className="text-center">
            <p className="text-sm text-base-content/70">Don't have an account? <a href="/register" className="link link-primary font-medium">Sign up here</a></p>
          </div>
        </div>
      </div>
    </div>
